...
def _limit_for(path: Path, start: int, first: bool, documents: bool) -> int:
    ...
    if path.suffix != '.py' and first and start == 1:
        return MODULE_LIMIT
...
def expand(names: List[str]) -> List[Path]:
    """ Read the files named, and the files under the directories named.
        ...
        Returns:
            paths (List[Path]):
                Every file to check, in a stable order.
    """
    ...
    for name in names:
        ...
        if path.is_dir():
            paths.extend(sorted(
                ...
                and str(child) not in SKIP
            ))
        elif name.endswith(SUFFIXES) and name not in SKIP:
            paths.append(path)
