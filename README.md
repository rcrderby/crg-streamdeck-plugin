...
| `Jam Control` | Starts a jam, stops a jam, or ends a timeout, and shows the relevant clock and jam data.  It turns amber, with dark text, five seconds before a jam should start, and pulses if the lineup runs past the time the ruleset gives it.  It turns red when the period has no time for another jam.  In overtime, it is red, then amber, then pulses.  It shows a faded `Start Jam` and takes no action while CRG will not start or stop a jam, such as after the official score is set. |
